...
VRChatイベント管理Bot で使用する各サービスの認証情報やリソースについて、有効期限・更新方法・注意事項をまとめたドキュメントです。
...
| GCP サービスアカウントキー（GCP認証用） | **無期限**（デフォルト） | なし | 低（定期ローテーション推奨） |
...
OAuth 同意画面が **テストモード** の場合:
- リフレッシュトークンは **発行から7日で失効** します
- **テストユーザーの上限は100アカウント**です（GCPのOAuth同意画面で管理）
...
**対応方法**: いずれの場合も `/カレンダー 認証` で再認証すれば復旧します。
...
## GCP サービスアカウントキー（GCP認証用）

Firestore、Secret Manager、Cloud Storage へのアクセスに使用されます（カレンダーAPIへのアクセスには OAuth 2.0 を使用します）。
...
| 機能 | 影響 |
|------|------|
| OAuth 新規認証（`/カレンダー 認証`） | **不可** — リダイレクト URI が機能しない |
...
- [ ] GCP サービスアカウントキーのローテーション（GCP認証用、推奨）
...
- [ ] `/カレンダー 認証状態` で認証が有効か確認
- [ ] 失効していた場合は `/カレンダー 認証` で再認証
