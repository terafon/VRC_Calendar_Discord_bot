...
| コマンド | 説明 |
|---------|------|
| `/色初期設定` | 繰り返しタイプごとのデフォルト色を一括設定（manage_guild権限必要） |
...
6. Discordに戻り、**`/色初期設定`** を実行して繰り返しタイプごとのデフォルト色を設定します
...
OAuth認証後、最初に `/色初期設定` を実行して5つの繰り返しカテゴリにデフォルト色を設定してください。
...
> **色初期設定が未完了の場合**: `/予定` コマンド実行時に `/色初期設定` を先に実行するよう案内されます。
...
```
/色追加 名前:特別イベント color_id:11 説明:特別イベント用
```
