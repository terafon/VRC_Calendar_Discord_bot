...
# 出力JSONスキーマ
{
  "action": "add|edit|delete|search|skip",
  "event_name": "予定名",
  "tags": ["タグ1", "タグ2"],
  "recurrence": "weekly|biweekly|nth_week|monthly_date|irregular",
  "nth_weeks": [2, 4],
  "monthly_dates": [5, 20],
  "event_type": "種類",
  "time": "14:00",
  ...
  "skip_date": "YYYY-MM-DD（skipアクション時: スキップする日付）",
  "start_date": "YYYY-MM-DD（editアクション時: 隔週イベントの開始日変更用）",
  "search_query": {
...
- weekday は weekly/biweekly/nth_week の場合は必須。
- irregular の場合は任意（主な開催曜日があれば設定、不明なら null）。
- monthly_date の場合は不要（null）。
...
# action=add の必須フィールド
- event_name: 予定名
- recurrence: 繰り返しパターン（weekly, biweekly, nth_week, monthly_date, irregular）
- weekday: 曜日（recurrence が monthly_date 以外の場合は必須。irregular の場合は任意で、主な開催曜日があれば設定、なければ null）
- monthly_dates: 毎月の開催日リスト（recurrence が monthly_date の場合は必須。例: [5, 20]）
- time: 開始時刻（HH:MM形式）
...
{{
  ...
  "event_data": {{
    ...
    "skip_date": "スキップする日付（YYYY-MM-DD形式） or null",
    "start_date": "隔週イベントの開始日変更（YYYY-MM-DD形式） or null"
  }}
}}
...
- 質問の順序: 開催頻度 → 曜日（monthly_dateの場合は開催日） → 時刻 → 所要時間 → タグ → 説明 → URL
- recurrence が monthly_date の場合は曜日の質問をスキップし、「何日に開催しますか？」と質問してください。回答を monthly_dates 配列に入れてください（例: [5, 20]）
- recurrence が irregular の場合は「主な開催曜日はありますか？（なければスキップして大丈夫です）」と任意で質問してください。ユーザーが「なし」「不明」「特にない」等と答えた場合は weekday を null のままにしてください。曜日を答えた場合はその曜日を weekday に設定してください。
...
- skip アクションの場合は event_name と skip_date が揃えば status: complete にしてください。不足している場合は質問してください（例: 「何日の予定をスキップしますか？」）。skip_date は YYYY-MM-DD 形式で出力してください。
- edit アクションで隔週イベントの開催タイミングを変更する場合は start_date を使用してください。start_date は YYYY-MM-DD 形式で、曜日が一致する日付である必要があります。recurrence を変更せず開催タイミングだけ変える場合に使います。
...
3. weekday（曜日）— 必須（recurrence が monthly_date の場合は不要。irregular の場合は任意で質問し、なければ null）
3a. monthly_dates（開催日）— recurrence が monthly_date の場合は必須（例: [5, 20]）
4. time（開始時刻）— 必須
...
def _build_server_context(server_context: Optional[Dict[str, Any]] = None) -> str:
    ...
    if events:
        ...
        recurrence_labels = {
            ...
            "nth_week": "第n週", "monthly_date": "毎月指定日",
            "irregular": "不定期",
        }
        ...
        for ev in events:
            ...
            detail_parts = []
            if rec_label:
                detail_parts.append(f"繰り返し:{rec_label}{nth_str}{md_str}")
            if wd_str:
...
class NLPProcessor:
    ...
    def _ensure_required_fields(self, result: Dict[str, Any]) -> Dict[str, Any]:
        """必須フィールドが揃っているか検証し、不足があればneeds_infoに強制変更"""
        action = result.get("action")
        status = result.get("status", "complete")

        # skip アクションのバリデーション
        if action == "skip" and status == "complete":
            event_data = result.get("event_data", {})
            missing = []
            if not event_data.get("event_name"):
                missing.append("予定名")
            if not event_data.get("skip_date"):
                missing.append("スキップする日付（例: 2026-03-15）")
            if missing:
        ...
        if recurrence and recurrence not in ("irregular", "monthly_date") and event_data.get("weekday") is None:
            missing_fields.append("曜日")
        if recurrence == "monthly_date" and not event_data.get("monthly_dates"):
            missing_fields.append("開催日（何日に開催するか）")
    ...
    def _validate_result(self, result: Dict[str, Any]):
        ...
        if result['action'] == 'skip':
            ...
            if 'skip_date' not in result:
                raise ValueError("スキップする日付が必要です")
        ...
        if result['action'] == 'add':
            ...
            if result['recurrence'] not in ('irregular', 'monthly_date') and 'weekday' not in result:
